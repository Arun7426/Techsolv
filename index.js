var outsideClick = function (e) {
   var container = getElementById(".customDrp")
   if (!container.is(e.target) && container.has(e.target).length === 0 && container.is(':visible')) {
      container.removeClass("active");
      document.getElementById(document).unbind("click", outsideClick);
   }
};
...
document.getElementById(document).ready(function)
{
   ...
   document.getElementsByClassName(".custmonDropDown").click function(e)   
   {
      if (document.getElementsByClassName(e.target).hasClass("crossString") || document.getElementsByClassName(e.target).hasClass("bowiro")) {
         return false;
      }

      if (getElementsByClassName(e.target).is("a")) {
         return true;
      }
      ...
      if (getElementsByClassName(e.target).hasClass("language") || getElementsByClassName(e.target).parent().hasClass("language")) {
         languageFunc();
      ...
      if (getElementsByClassName(e.target).hasClass("searchCoinDrp")) {
         document.getElementById('tokensearch1').focus();
      }

      if (getElementById(this).next(".customDrp").length) {
         getElementsByClassName(this).next(".customDrp").addClass("active");
      } else {
         getElementsByClassName(this).children(".customDrp").addClass("active");
      }

      e.stopPropagation();
      getElementsByClassName(document).bind("click", outsideClick);
   };
   ...
   let basic_banner_read_btn = false; 
      document.getElementById(".basic_banner_read_btn").addEventListener("click")
   {
      if (basic_banner_read_btn == false) {
         getElementById(this).text("Read Less");
         getElementsByClassName(".basic_banner_text").slideToggle(500);
         basic_banner_read_btn = true;
      } else {
         getElementsByClassName(".basic_banner_text").slideToggle(500);
         getElementById(this).text("Read More");
   ...
   document.getElementById(".filterToggelBtn").addEventListener("click")
   {
      getElementById(".filterToggel").slideToggle("active");
   };

   // ==  .hideCustomDrp == //
   
   document.getElementById(".hideCustomDrp").addEventListener("click")
   {
      ...
      function (e) {
         ...
         if (
            ...
            if (target.hasAttribute("data-target")) {
               ...
               if (m_ID == "LoginBox") {
                  getElementById("#login_error").css("display", "none");
                  getElementById("#login_error").html("");
               } else if (m_ID == "signUpBox") {
                  getElementById("#signup_error").css("display", "none");
                  getElementById("#signup_error").html("");
               } else if (m_ID == "ResetPassword") {
                  getElementById("#fpass_error").css("display", "none");
                  getElementById("#fpass_error").html("");
   ...
   {
      getElementById(".customPopup").removeClass("active");
      getElementById(".commonPopup").removeClass("active");
   ...
   document.getElementById(".mainTable").on("scroll", function (e))
   {
      let isScroll = e.currentTarget.scrollLeft;
      if (isScroll) {
         getElementById(".ListingTable th:nth-child(3), td:nth-child(3)").addClass("before");
      } else {
         getElementById(".ListingTable th:nth-child(3), td:nth-child(3)").removeClass("before");
      }
   };
   

   // ==  responsiveTable currenciesTable == //
   document.getElementById(".currenciesTable, .exchangeTable").on("scroll", function (e) {
      let isScroll = e.currentTarget.scrollLeft;
      if (isScroll) {
         getElementById(".ListingTable th:nth-child(2), td:nth-child(2)").addClass("before");
      } else {
         getElementById(".ListingTable th:nth-child(2), td:nth-child(2)").removeClass("before");
      }
   });

   // .historical-data
   document.getElementById(".historical-data").on("scroll", function (e) {
      let isScroll = e.currentTarget.scrollLeft;
      if (isScroll) {
         getElementById(".ListingTable th:nth-child(1), td:nth-child(1)").addClass("before");
      } else {
         getElementById(".ListingTable th:nth-child(1), td:nth-child(1)").removeClass("before");
      }
   });

   // .financialTable
   document.getElementById(".financialTable").on("scroll", function (e) {
      let isScroll = e.currentTarget.scrollLeft;
      if (isScroll) {
         getElementById(".ListingTable th:nth-child(1), td:nth-child(1)").addClass("before");
      } else {
   ...
   // searchPopup
   document.getElementById(".header-for-mobile .searchPopup").addEventListener("click") {
      getElementById("body").css({
         overflow: "hidden",
      });
      getElementById(".searchDiv-for-mb").addClass("active");
   };

   document.getElementById(".searchpopupHider").addEventListener("click") {
      getElementById("body").css({
         overflow: "auto",
      });
      getElementById(".searchDiv-for-mb").removeClass("active");
   };

   // .gototop Icon Div
   addEventListener() {
      document.getElementById(window).addEventListener("scroll") {
         var scrolled = getElementById(window).scrollTop();
         if (scrolled > 200) getElementById(".gototopIconDiv").fadeIn("slow");
         if (scrolled < 200) getElementById(".gototopIconDiv").fadeOut("slow");
      };

      document.getElementById(".gototopIconDiv").addEventListener("click") {
         getElementById("html, body").animate({
            scrollTop: "0",
         },
            500
         );
   ...
   document.getElementById(".popupRemover").addEventListener("click") {
      getElementById("#LoginBox").removeClass("active");
      getElementById("#signUpBox").removeClass("active");
   };
   ...
   let isHighLight = getElementById("#HighlightsBox");
   isHighLight.click(function () {
      getElementsByClassName(".Banner_cards").slideToggle(100);
   });
};
...
// == == == faq scripts == == == //
document.getElementById(".faqAcc .flexAccHead").addEventListener("click") {
   let AccHeadObj = getElementById(this);
...
function toggleCurrencyMoreData(elem) {
   ...
   if (currentstate == alternate) {
      getElementById(".currencyDetail .bottomListing").css("display", "none");
      elem.innerHTML = toggleCurrencyMoreDataState;
   } else {
      getElementById(".currencyDetail .bottomListing").css("display", "block");
      elem.innerHTML = alternate;
   }
}
